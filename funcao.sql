create or replace function cria_transacao(id_cliente int, t_valor bigint, t_tipo varchar, t_descricao varchar) returns table(l bigint, s bigint)
as $$
declare
	novoSaldo bigint;
	novoValor bigint;
...
--		raise exception 'id inexistente';
		return query select cast(-1 as BIGINT), cast(-1 as BIGINT);
	end if;
	if t_tipo = 'd' then
		novoValor := t_valor * -1;
	else
...
--		raise exception 'limite insuficiente';
		return query select cast(-2 as BIGINT), cast(-2 as BIGINT);
	else
